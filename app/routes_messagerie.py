...
@login_required
def messages():
    # Récupérer tous les contacts uniques avec lesquels l'utilisateur a échangé des messages
    # (envoyés ou reçus)
    ...
    # Ajouter les contacts des demandes acceptées (même sans message)
    ...
    # Trier les contacts par dernier message (plus récent en premier)
    contacts_avec_dernier_msg = []
    for contact in contacts:
        ...
        contacts_avec_dernier_msg.append((contact, dernier_msg))
    
    # Trier par date du dernier message (les plus récents en haut)
    contacts_avec_dernier_msg.sort(key=lambda x: x[1].date_envoi if x[1] else datetime.min, reverse=True)
    contacts = [c for c, _ in contacts_avec_dernier_msg]
    
    # Auto-ouvrir depuis ?contact=ID
    contact_id = request.args.get("contact", type=int)
    contact_ouverture = User.query.get(contact_id) if contact_id and contact_id != current_user.id else None
...
def gerer_message(data):
    ...
    if not contenu:
        return
    msg = Message(sender_id=sender_id, receiver_id=receiver_id, contenu=contenu,
                  date_envoi=datetime.utcnow(), lu=False)
    db.session.add(msg)
    db.session.commit()
